   ...
   static function beforeUpdate(ChangeTask $task) {
      ...
      if ($config->getField('is_changetasktodo')) {
         ...
         if ($change->getFromDB($task->fields['changes_id'])) {
            if (in_array($change->fields['status'], array_merge(Change::getSolvedStatusArray(),
                                                                Change::getClosedStatusArray()))) {
               Session::addMessageAfterRedirect(__("You cannot change status of a task in a solved change",
